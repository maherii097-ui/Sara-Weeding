...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (musicBtn && heroSection) {
    const heroObserver = new IntersectionObserver((entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          musicBtn.classList.remove('is-hidden');
        } else {
          musicBtn.classList.add('is-hidden');
        }
      });
    }, { threshold: 0.2 });
  ...
  /* ---------- 5) Comments (مباركات الأحبة) — مشتركة بين كل الزوار عبر Cloud Firestore ---------- */
  ...
  const commentsRef = SITE_CONFIG.db ? SITE_CONFIG.db.collection('comments') : null;
  ...
  if (commentsRef) {
    console.log('[تشخيص] بدأ الاستماع لمجموعة comments في Cloud Firestore...');
    commentsRef.orderBy('ts', 'desc').onSnapshot((snapshot) => {
      list.innerHTML = '';
      if (snapshot.empty) {
        renderEmptyState();
        return;
      ...
      snapshot.forEach((doc) => {
        const data = doc.data();
        addCommentToDOM({ name: data.name, message: data.message }, { animate: false });
      });
    }, (err) => {
      ...
      if (err && err.code === 'PERMISSION_DENIED') {
        ...
        setStatus('قواعد الأمان في Cloud Firestore ترفض القراءة حاليًا. راجع Firebase Console → Firestore Database → Rules.', 'error');
      } else {
  ...
  form.addEventListener('submit', (e) => {
    ...
    setStatus('', null);
    console.log('[تشخيص] جاري إرسال البيانات إلى Cloud Firestore...');

    const hangWarning = setTimeout(() => {
      console.warn('[تشخيص] الطلب مستني من أكتر من 8 ثواني من غير رد — راجع قواعد الأمان في Firestore أو الشبكة.');
      setStatus('الطلب واخد وقت أطول من الطبيعي… تأكد من قواعد الأمان في Firebase Console.', 'error');
    }, 8000);

    commentsRef.add({ name, message, ts: Date.now() })
      .then(() => {
      ...
      .catch((err) => {
        ...
        if (err && err.code === 'PERMISSION_DENIED') {
          setStatus('قواعد الأمان في Cloud Firestore ترفض الكتابة حاليًا. راجع Firebase Console → Firestore Database → Rules.', 'error');
        } else {
